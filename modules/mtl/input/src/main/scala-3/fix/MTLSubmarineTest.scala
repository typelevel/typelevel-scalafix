...
object MTLSubmarineTest {
  ...
  // select the handler form and effect type; this file selects Scala 3; an `assert`
  ...
  object EffectPropagation {
    ...
    def conditional[F[_]: Async](condition: Boolean)(using r: Raise[F, String]): F[Unit] = {
    ...
    def blocks[F[_]: Async](using r: Raise[F, String]): F[Unit] = {
      ({
        Async[F].unit
        methodRaise[F]
      }).attempt // assert: TypelevelMTLSubmarine.mtlSubmarineErrorHandling
